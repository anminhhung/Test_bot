...
class PandasCSVReaderCustomised(BaseReader):
    ...
    def load_data(
        ...
        text_list = df.apply(lambda row: " ".join([ f"{k}: {v}" for k,v in row.to_dict().items()]), axis=1).tolist()
        ...
        logger.info(f"Chunked {file} with shape {df.shape} into {len(text_list)} parts. with top_k={self._top_k}")
