...
class ContextualRAG:
    ...
    def __init__(self, setting: GlobalSettings):
        ...
        self.qdrant_client = QdrantVectorDatabase(
            ...
            port=setting.qdrant_config.port,
        )
    ...
    def split_document(
        ...
        logger.info(f"Splitting document with {len(document)} chunks ...")
    ...
    def contextual_rag_search(
        ...
        combined_nodes: list[NodeWithScore] = [
            NodeWithScore(
                node=TextNode(text=point.payload["text"]),
                score=point.score,
            )
        ...
        # logger.info(f"Nodes: {contexts}")

        logger.debug(f"Generating response ... for {len(contexts)} contexts")
        messages = [
            ChatMessage(
                role="system",
